...
def generate_indices(start_index=0, n=4):
...
raw_selected_1 = data[:, indices_1, :, :, 0]  # (818, 4, 52, 8)
print("Shape finale de la matrice extraite1 :", raw_selected_1.shape)
print(raw_selected_1[0, 0, :, 0])

# Transposition vers (4, 8, 52, 818)

selected_1 = raw_selected_1.transpose(0, 3, 2, 1)  # (818, 8, 52, 4)

# Extraction 2ème AP : symboles à partir de l’indice 64
indices_2 = generate_indices(start_index=64)
raw_selected_2 = data[:, indices_2, :, :, 0]  # (818, 4, 52, 8)
print("Shape finale de la matrice extraite2 :", raw_selected_2.shape)
print(raw_selected_2[0, 0, :, 0])
# Transposition
selected_2 = raw_selected_2.transpose(0, 3, 2, 1)  # (818, 8, 52, 4)

#########################################################################

# Concaténation sur l’axe des sous-porteuses (axe=2)
combined_matrix = np.concatenate([selected_1, selected_2], axis=2)  # (4, 8, 104, 818)

#########################################################################
# Sauvegarde
output_dir = 'matrix2_4_8_104_818'
os.makedirs(output_dir, exist_ok=True)

output_path = os.path.join(output_dir, 'matrix_4_8_104_818.npy')
