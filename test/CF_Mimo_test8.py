...
# ------  OFDM & Sous porteuse
BG = 0
Mfft = 818 #Size of the FFT (Fast Fourier Transform)
Mqam = 4
modulation_type = "qam"
...
for UOIindex in range(0,8):
    # for SCOIindex in range(0,818):
        ...
        # ---- PZF Analytique ----- # 
        is_strong = np.zeros((L,K),dtype=int)
        # Theoretical SINR and SE computation
        for l in range(L):
            for k in range(K):
                if np.any(group_strong_UE[l] == k):
                    is_strong[l,k] = 1
                else :
                    is_strong[l,k] = 0
                    
            
        CPktheo0 = np.sqrt((M - is_strong[:,UOIindex] * Tau_ss1[:])*(RHOlk[:,UOIindex]) * NUlk[:,UOIindex])       
        CPktheo = (np.sum(CPktheo0,axis=0))**2
        CPktheo_K0 =    np.sqrt((M - is_strong[:,UOIindex] * Tau_ss1[:])*(RHOlk[:,UOIindex]**2) * NUlk[:,UOIindex])         
        CPktheo_K0 = (np.sum(CPktheo_K0,axis=0))**2
        
        PUktheo0 = 0
        PUktheo0_K0 = 0
        for t in range(0,len(UONIindex)):
            PUktheo0 = PUktheo0 + (np.sum(np.sqrt( (M - (is_strong[:,t]*Tau_ss1[:])) * RHOlk[:,int(UONIindex[t])]*NUlk[:,UOIindex])))**2     
            PUktheo0_K0 =  PUktheo0_K0 + (np.sum(np.sqrt( (M - (is_strong[:,t]*Tau_ss1[:])) * (RHOlk[:,int(UONIindex[t])]**2)*NUlk[:,UOIindex])))**2     
        PUktheo = PUktheo0    
        PUktheo_K0 = PUktheo0_K0
        
        UIkttheo0=0
        UIkttheo0_K0 = 0
        for t in range(0,K):
            UIkttheo0 = UIkttheo0 + RHOlk[:,t]*(BETAlk[:,UOIindex]- is_strong[:,UOIindex] * is_strong[:,t] * NUlk[:,UOIindex])   
            UIkttheo0_K0 = UIkttheo0_K0 + (RHOlk[:,t]**2)*(BETAlk[:,UOIindex]- is_strong[:,UOIindex] * is_strong[:,t] * NUlk[:,UOIindex])   
        UIkttheo = np.sum(UIkttheo0)
        UIkttheo_K0 = np.sum(UIkttheo0_K0)
        ...
        for iNreal in range(0,Nreal):  
            if iNreal % 5 == 0:
                print("Progress : ", np.round((iNreal/Nreal*100),2), " % ...")
        ...
        data_to_save = {
            "UOIindex": int(UOIindex),
            "SCOIindex": int(SCOIindex),
            "SINR_simule_PZF": float(SINRksim_PZF),
            "SE_simule_PZF": float(SEksim_PZF),
            "SINR_theorique_PZF": float(SINRktheo_PZF),
            "SE_theorique_PZF": float(SEkTheo_PZF),
        }


        # Nom du fichier JSON
        file_name = "simulation_data_test_8.json"
